...
def calculate_disparity_map(left_img: torch.Tensor,
  ...
  for i in range(left_img.shape[0] - block_size + 1):
        for j in range(left_img.shape[1] - block_size + 1):
            left_patch = left_img[i:i + block_size, j:j + block_size]
            min_error = np.inf
            min_disparity = 0
            for d in range(max_search_bound):
                if j - d < 0:
                    break
                right_patch = right_img[i:i + block_size, j - d:j - d + block_size]
  ...
  for i in range(H - block_size + 1):
      for j in range(W - block_size + 1):
          ...
          for d in range(max_disparity):
              if j - d < 0:
                  cost_volume[i, j, d] = 255
              else:
                  right_patch = right_img[i:i + block_size, j - d:j - d + block_size]
                  cost_volume[i, j, d] = sim_measure_function(left_patch, right_patch)
  ############################################################################
  # Student code end
  ############################################################################
  return cost_volume
