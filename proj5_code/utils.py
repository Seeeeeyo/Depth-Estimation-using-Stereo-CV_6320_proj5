...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  # Student code begin
  ############################################################################
  im_left = torch.randint(0, 2, (H, W), dtype=torch.float32)

  im_right = im_left.clone()

  center_y, center_x = H // 2, W // 2

  block_start_y, block_start_x = center_y - block_size[0] // 2, center_x - block_size[1] // 2
  block_end_y, block_end_x = block_start_y + block_size[0], block_start_x + block_size[1]

  block = im_right[block_start_y:block_end_y, block_start_x:block_end_x]
  im_right[block_start_y:block_end_y, block_start_x - disparity:block_end_x - disparity] = block

  im_right[block_start_y:block_end_y, block_end_x - disparity:block_end_x] = torch.randint(0, 2,
                                                                                           (block_size[0], disparity),
                                                                                           dtype=torch.float32)

  im_left = im_left.unsqueeze(-1).repeat(1, 1, C)  # Add channel dimension
  im_right = im_right.unsqueeze(-1).repeat(1, 1, C)
  ############################################################################
  # Student code end
  ############################################################################
